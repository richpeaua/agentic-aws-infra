...
- [x] v2.5 - Handoff hardening and cost controls (epic #33): widened writable allowlist for repo-owned verification (#34); truthful `success`/`failed`/`incomplete` finalization (#35); runtime/budget guards - native `--max-budget-usd` cap plus a portable wall-clock timeout (#36); live `stream-json` progress and usage visibility (#37); safe GitHub body construction and the implementer permission/guard docs (#38). No change to the deployed AWS footprint.
